...
# -*- coding: utf-8 -*-
"""
sr3le_extract.py — Saints Row: The Third REMASTERED .le_strings 解包工具
====================================================================
背景：官方 ThomasJepp.SaintsRow.ExtractStrings.exe (2013) 仅支持 SR3 原版，
      原版 StringBucket 为 8 字节 {count, offset}。
      Remastered (2020) 的 StringBucket 扩为 16 字节 {count, pad, offset, pad}，
      旧工具错位读取导致 "已添加了具有相同键的项" 崩溃。
本工具按 Remastered 布局解析，并做 charlist 字符解码 + xtbl 键名反查。

用法:
  python sr3le_extract.py <misc目录> <misc_tables目录> <输出目录>
...
def parse_le_strings(path, charmap):
    ...
    布局:
      ...
      offset 表   : 从 bucket.offset 起, count 个 8B 条目, 每个 = {字符串绝对偏移 u32, 填充零 u32}
                    (注意: 条目为 8 字节步长, 4 字节步长会把填充零当空槽跳掉、漏一半条目)
...
def main():
    ...
    for le in le_files:
        ...
        # zh 的 charlist 文件实际命名是 charlist_zh.dat (官方规则 ch 不存在)
