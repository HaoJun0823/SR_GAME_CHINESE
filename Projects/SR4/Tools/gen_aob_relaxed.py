...
DLLMAIN = os.path.join(ROOT, "SR4R_I18N", "dllmain.cpp")
INC_PATH = os.path.join(ROOT, "SR4R_I18N", "aob_l2_arrays.inc")
DEFAULT_GAME = r"I:\SteamLibrary\steamapps\common\Saints Row IV"
DEFAULT_MS_DUMP = os.path.join(ROOT, ".temp", "dump_ms", "SR4R_dump_text.bin")
BASE = 0x140000000
...
#   这也是 L3 存在的唯一理由 —— 其余 6 条 MS 上 L1 照常命中(日志已验证)。
L3_HOOKS = {"Format", "Subtitle"}
...
def main():
    ...
    for hook, ln, msk, l1h, l2h, l3h, drift in results:
        ...
        #   MS 构建   : 换代 hook(Format/Subtitle) L1 必须落空且 L3 恰好 1 命中;
        #               其余 6 条 L1 必须恰好 1 命中
    ...
    w("      MS Store 要求 —— 换代 hook(Format/Subtitle) L1 落空 + L3 恰好 1 命中,")
    w("      其余 6 条 L1 恰好 1 命中。")
